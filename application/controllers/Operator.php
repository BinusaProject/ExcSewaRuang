 <?php
    ...
    class operator extends CI_Controller
    {
        ...
        public function aksi_tambah_ruangan()
        {
            ...
            // Validasi no_lantai
            if (empty($no_lantai) || !is_numeric($no_lantai) || strpos($no_lantai, '-') !== false) {
                $errors[] = 'Nomor Lantai harus diisi dengan angka dan tidak boleh mengandung karakter "-".';
            }

            // Validasi no_ruang
            if (empty($no_ruang) || !preg_match('/^[0-9]+$/', $no_ruang) || strpos($no_ruang, '-') !== false) {
                $errors[] = 'Nomor Ruang harus diisi dengan angka dan tidak boleh mengandung karakter "-".';
            }

            // Validasi harga
            if (empty($harga) || !is_numeric($harga) || $harga < 0) {
                $errors[] = 'Harga harus diisi dengan angka positif dan tidak boleh kosong.';
            }
        ...
        public function aksi_edit_ruangan($id)
        {
            ...
            $response = [
                ...
                'redirect' => base_url('operator/ruang/edit_ruangan/' . $id), // Redirect URL setelah berhasil atau gagal
            ];
            ...
            if ($id) {
                ...
                if ($current_data) {
                    ...
                    // Validasi no_lantai
                    if (!empty($no_lantai) && is_numeric($no_lantai) && strpos($no_lantai, '-') === false) {
                        if ($no_lantai !== $current_data->no_lantai) {
                            $changes_made = true;
                    ...
                    // Validasi no_ruang
                    if (!empty($no_ruang) && preg_match('/^[0-9]+$/', $no_ruang) && strpos($no_ruang, '-') === false) {
                        if ($no_ruang !== $current_data->no_ruang) {
                            $changes_made = true;
                    ...
                    if (!empty($image) && isset($file_info['extension'])) {
                        $extension = strtolower($file_info['extension']);

                        if (in_array($extension, $allowed_extensions) && strpos($image, '-') === false) {
                            $changes_made = true;
                            ...
                            if (move_uploaded_file($foto_temp, $upload_path)) {
                                ...
                                $response = [
                                    'status' => 'error',
                                    'message' => 'Gagal mengunggah gambar. Silakan coba lagi.',
                                    'redirect' => base_url('operator/edit_ruangan/' . $id), // Redirect ke halaman edit jika gagal
                                ];

                                // Langsung redirect dengan SweetAlert
                                echo '<script>
                                Swal.fire({
                                    icon: "error",
                                    title: "Error",
                    ...
                    // Jika setidaknya satu bidang telah diubah, tampilkan SweetAlert, baru lanjutkan proses penyimpanan
                    if ($changes_made) {
                        $response['status'] = 'info';
                        $response['message'] = 'Anda yakin ingin menyimpan perubahan?';
                        $response['confirmButton'] = true;
                        $response['confirmButtonText'] = 'Ya, Simpan Perubahan';
                        $response['cancelButton'] = true;
                        $response['cancelButtonText'] = 'Tidak, Batalkan';
                        $response['data'] = $data;
                        ...
                        $response = [
                            ...
                            'redirect' => base_url('operator/edit_ruangan/' . $id), // Redirect ke halaman edit jika tidak ada perubahan
                        ];
