...
class operator extends CI_Controller
{
    ...
    public function export_pdf($id)
    {
        ...
        $peminjaman = $this->m_model->get_peminjaman_by_id($id);

        // Check if data is found
        if (!$peminjaman) {
            show_404(); // or handle the case when data is not found
        }

        // Fetch additional data as needed
        $tambahan = $this->m_model->get_tambahan_by_id($peminjaman->id_tambahan);

        $data['peminjaman'] = $peminjaman;
        $data['tambahan'] = $tambahan;

        // Load PDF library and view
